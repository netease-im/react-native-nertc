...
workspace_name="NertcExample"

# .xcodeproj的名字，如果is_workspace为false，则必须填。否则可不填
project_name="NertcExample"

# 指定项目的scheme名称（也就是工程的target名称），必填
scheme_name="NertcExample"
...
mobileprovision_name="enterpriseForWildCard"

# 项目的bundleID，手动管理Profile时必填
bundle_identifier="com.netease.nmc.NERtcSample-ScreenShare-iOS-Objective-C"

# 自定义参数
GCC_PROGRESS_DEFINITIONS='$(inherited)'
ASAN_FLAG=""
for i in "$@"; do
    if [ $i = "--asan" ]; then      ASAN_FLAG="-enableAddressSanitizer YES "; fi
    if [ $i = "--audioOnly" ]; then GCC_PROGRESS_DEFINITIONS='$(inherited)'" RTC_AUDIO_ONLY"; fi
done
...
# 删除之前老的 ipa 
ipa_path="${project_dir}/ipa"
echo "${ipa_path}"
if [ -d "${ipa_path}" ] ; then
    echo "${ipa_path}目录存在，进行删除"
    rm -r ${ipa_path}
fi
...
if $is_workspace ; then
    ...
    xcodebuild archive -workspace ${workspace_name}.xcworkspace \
                    ...
                    GCC_PREPROCESSOR_DEFINITIONS="$GCC_PROGRESS_DEFINITIONS" \
                    $ASAN_FLAG
else
    # 编译前清理工程
...
# 产物ipa包拷贝到指定目录
echo "开始拷贝ipa产物${ipa_name}.ipa和对应的dsym文件,首先删除旧包路径"
rm -rf $project_dir/out/demo
mkdir -p $project_dir/out/demo
cp -rf $project_dir/ipa/$ipa_name.ipa $project_dir/out/demo/$ipa_name.ipa
cp -rf $export_archive_path/dSYMs/NertcExample.app.dSYM $project_dir/out/demo
cp -rf $export_archive_path/dSYMs/NertcExample-Broadcast.appex.dSYM $project_dir/out/demo
cp -rf $export_archive_path/dSYMs/NertcExample-BroadcastSetupUI.appex.dSYM $project_dir/out/demo

# 压缩
cd out
tar czvf NERtcReactNative.tar.gz demo
